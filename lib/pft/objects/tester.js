...
PFT.tester = {
    ...
    executionLoop: function () {
        var duration = (PFT.tester._tests[0] && PFT.tester._tests[0].maxDuration) ? PFT.tester._tests[0].maxDuration : PFT.tester.timeOutAfter;
        if (PFT.tester._tests[0] && PFT.tester._tests[0].startTime && (new Date().getTime() - PFT.tester._tests[0].startTime) >= duration) {
            var msg = "Test '" + PFT.tester._tests[0].name + "' exceeded timeout of " + duration;
            PFT.tester._tests[0].errors.push(msg);
            PFT.logger.log(PFT.logger.TEST, msg);
            PFT.tester.onTimeout({ test: PFT.tester._tests[0], message: msg });
            PFT.tester.done();
        }
        if (PFT.tester.inQueue.length > 0) {
            if (PFT.tester.ready && PFT.tester.running) {
                try {
                    ...
                    PFT.tester._tests.unshift(PFT.tester.inQueue.shift());
                    PFT.tester._tests[0].startTime = new Date().getTime();
                    if (PFT.tester._tests[0].type === PFT.tester.TEST) {
                        PFT.tester._tests[0].page = PFT.createPage();
                    }
                    PFT.tester._tests[0].data = PFT.tester._tests[0].data;
                    PFT.tester._tests[0].fn.call(this, PFT.tester._tests[0].page, PFT.tester._tests[0].data, PFT.tester.assert);
                } catch(e) {
                    phantom.onError(e);
                }
            }
     ...
     */
        ...
        if (testObj.type === PFT.tester.TEST) {
            PFT.tester.remainingCount--;
            testObj.page.close();
            PFT.tester.onTestCompleted({ test: PFT.tester._tests[0] });
        }

        MutexJs.release(testObj.lockId);
    },
